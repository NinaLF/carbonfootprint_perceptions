...
```{r}
data.regression.total$rel_regression.weight <- scale(data.regression.total$regression.weight)[,1]
data.regression.total$rel_regression.weight.f <- cut(data.regression.total$rel_regression.weight, breaks=c(-5, -1,-0.5, 0.5, 1,6))

data.regression.total$rel_regression.weight.f <- factor(data.regression.total$rel_regression.weight.f ,
                                             levels=c("(-5,-1]" , "(-1,-0.5]", "(-0.5,0.5]", "(0.5,1]",  "(1,6]" ) ,
                                             labels=c("-much lower than average", "lower than average", 
                                                      "average", "higher than average", "much higher than average"))

ggplot(data.regression.total, aes(x=reported.behavior, fill=rel_regression.weight.f)) +
    geom_density(alpha=.7) +
    theme_minimal() + labs(alpha=NULL, x="carbon footprint of reported behavior", fill="impact perception") +
    scale_fill_manual(values=c(  "#a50000", "#DC582A","#F6BE00", "#2F8D06", "#127d71"))
