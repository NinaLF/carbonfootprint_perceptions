...
data.accuracy.score$income <- factor(data.accuracy.score$income, levels=c("< 18.000£", "18.000£ to 23.000£", "23.001£ to 30.500£", "30.501£ to 45.000£", "> 45.001£"))
data.accuracy.score$rel_cc_concern.mean.f <- factor(data.accuracy.score$rel_cc_concern.mean.f, levels=c("below average", "average", "above average"))
data.accuracy.score$rel_cor.f <- factor(data.accuracy.score$rel_cor.f, levels=c("below average", "average", "above average"))
data.accuracy.score <- data.accuracy.score %>% rename( correlation = estimate)
...
data.accuracy.score.excl <- data.accuracy.score %>% filter(!correlation<0) 
...
### Predicting Accuracy 
```{r}
model.accuracy2 <- lm(correlation ~ age + gender + education + income +
                       cc_concern.mean + conservative_liberal ,
                     data=data.accuracy.score)
tab_model(model.accuracy2)
model.accuracy2.excl <- lm(correlation ~ age + gender + education + income +
                       cc_concern.mean + conservative_liberal ,
                     data=data.accuracy.score.excl)
...
```{r}
ggplot(data.accuracy.score.excl, aes(x=correlation, fill=rel_cc_concern.mean.f, alpha=.3)) +
  geom_density(alpha=.5) +  labs(alpha=NULL, fill= "cc concern", x="accuracy") +  
  theme_minimal() +
  scale_fill_manual(values=c(  "#a50000", "#F6BE00", "#2F8D06"))
...
```{r}
ggplot(data.accuracy.score.excl, aes(x=correlation, fill=income)) +
    geom_density(alpha=.7) +
    theme_minimal() + labs(alpha=NULL, x="accuracy") +
    scale_fill_manual(values=c(  "#6ec19d", "#0c8853", "#127d71", "#12547d", "#2e519a")) +
...
```{r}
model.support.accuracy2 <- lm(policy.support.total ~ correlation+ conservative_liberal +
                               age + gender + education + income +
                               cc_concern.mean ,
                             data=data.accuracy.score)
tab_model(model.support.accuracy2)

model.support.accuracy.excl <- lm(policy.support.total ~ correlation+ conservative_liberal +
                                  age + gender + education + income +
                                  cc_concern.mean ,
                              data=data.accuracy.score.excl)
tab_model(model.support.accuracy.excl)

model.support.accuracy2.mod <- lm(policy.support.total ~ correlation*rel_cc_concern.mean+ conservative_liberal+
                               age + gender + education + income ,
                             data=data.accuracy.score.excl)
...
```{r}
ggplot(data.accuracy.score.excl, aes(x=policy.support.total, fill=rel_cor.f)) +
  geom_density(alpha=.5) +  labs(alpha=NULL,  fill="accuracy", x="policy support") +
  theme_minimal() +
  scale_fill_manual(values=c(  "#a50000", "#F6BE00", "#2F8D06")) +
    theme(plot.caption = element_text(hjust = 0),
        plot.title.position = "plot",
        ...
        axis.text.x = element_text(size = 23, family = 'Arial', color="black"),
        legend.title = element_text(size = 22, face="plain",family = 'Arial', color="black"),
        legend.text =  element_text(size = 22, face="plain",family = 'Arial', color="black"),
        strip.text = element_text(size = 22, face="plain",family = 'Arial', color="black")
        )
...
model.behavior.accuracy2 <- lm(total_co2 ~ correlation + cc_concern.mean+ conservative_liberal +
                                age + gender + education + income ,
                              data=data.accuracy.score)
tab_model(model.behavior.accuracy2)

model.behavior.accuracy2.excl <- lm(total_co2 ~ correlation + cc_concern.mean+ conservative_liberal +
                                   age + gender + education + income ,
                               data=data.accuracy.score.excl)
tab_model(model.behavior.accuracy2.excl)
# no sign interaction
model.behavior.accuracy2.mod <- lm(total_co2 ~ rel_cor*rel_cc_concern.mean+ conservative_liberal +
                                   age + gender + education + income ,
                               data=data.accuracy.score.excl)
...
#  tapply(data.accuracy.score.excl$total_co2, data.accuracy.score.excl$rel_cor.f, mean)
#data.accuracy.score.income2 <- data.accuracy.score.excl %>% filter(income=="< 18.000£" | income=="18.000£ to 23.000£" | income=="23.001£ to 30.500£")

ggplot(data.accuracy.score.excl, aes(x=total_co2, fill=rel_cor.f)) +
  geom_density(alpha=.5) + labs(x="carbon footprint of reported behavior", fill="accuracy") +
  theme_minimal() +
  scale_fill_manual(values=c(  "#a50000", "#F6BE00", "#2F8D06"))


data.accuracy.score.excl$rel_total_co2 <- scale(data.accuracy.score.excl$total_co2 )[,1]
data.accuracy.score.excl$rel_total_co2.f <- cut(data.accuracy.score.excl$rel_total_co2, breaks=c(-2, -1,-0.5, 0.5, 1,6))

data.accuracy.score.excl$rel_total_co2.f <- factor(data.accuracy.score.excl$rel_total_co2.f ,
                                             levels=c("(-2,-1]" , "(-1,-0.5]", "(-0.5,0.5]", "(0.5,1]",  "(1,6]" ) ,
                                             labels=c("-much lower than average", "lower than average", 
                                                      "average", "higher than average", "much higher than average"))

ggplot(data.accuracy.score.excl, aes(x=correlation, fill=rel_total_co2.f, alpha=.3)) +
  geom_density() + labs(x="accuracy", fill="carbon footprint of reported behavior") +
  theme_minimal() +
  scale_fill_manual(values=c("#127d71", "#2F8D06","#F6BE00","#DC582A" , "#a50000"))
...
model.behavior.accuracy1.car <- lm(footprint.travel.car ~   cc_concern.mean+ conservative_liberal + correlation + # policy+
                                age + gender + education + income ,
                              data=data.accuracy.score.excl)
model.behavior.accuracy1.fly <- lm(footprint.flying ~   cc_concern.mean+ conservative_liberal  + correlation + # policy+
                                age + gender + education + income ,
                              data=data.accuracy.score.excl)

model.behavior.accuracy1.diet <- lm(footprint.diet ~    cc_concern.mean+  conservative_liberal  + correlation +  # policy+
                                age + gender + education + income ,
                              data=data.accuracy.score.excl)

model.behavior.accuracy1.elec <- lm(footprint.electricity ~  +cc_concern.mean+ conservative_liberal  + correlation +  # policy+
                                age + gender + education + income ,
                              data=data.accuracy.score.excl)

model.behavior.accuracy1.reg <- lm(footprint.regional ~  cc_concern.mean+ conservative_liberal  + correlation +  # policy+
                                age + gender + education + income ,
                              data=data.accuracy.score.excl)
...
data.regression.weights.long_clean <-  data.regression.weights.long %>% select(participant.label, behavior, regression.weight, gender, age, income, education, cc_concern.mean, conservative_liberal)
data.regression.weights.long_clean$behavior <- factor(data.regression.weights.long_clean$behavior, levels=c("commute.weight", "diet.weight", "electricity.weight", "flying.weight", "recycling.weight", "regional.weight" ),
                                                      labels=c("commute", "diet", "electricity", "flying", "recyling", "regional") )

data.regression.weights.policies <- data.regression.weights.long %>% select(participant.label, policy.car, policy.flying, policy.RE, policy.meat,   policy.recycle, policy.regional_food )
data.regression.weights.policies <- unique(data.regression.weights.policies)

data.regression.weights.policies.long <- pivot_longer(data.regression.weights.policies,policy.car:policy.regional_food,  names_to = "behavior", values_to = "policy.support")
data.regression.weights.policies.long$behavior <- factor(data.regression.weights.policies.long$behavior, 
                                                         levels=c( "policy.car", "policy.meat", "policy.RE","policy.flying", "policy.recycle", "policy.regional_food" ),
                                                         labels=c("commute", "diet", "electricity", "flying", "recyling", "regional") )  
  
data.regression.weights.footprint <- data.regression.weights.long %>% select(participant.label, footprint.public.transport, footprint.flying, footprint.electricity, footprint.diet,   footprint.recycling, footprint.regional )
data.regression.weights.footprint <- unique(data.regression.weights.footprint)

data.regression.weights.footprint.long <- pivot_longer(data.regression.weights.footprint, c(footprint.public.transport,footprint.diet, footprint.regional, footprint.electricity, footprint.flying,footprint.recycling),
                                                                                            names_to = "behavior", values_to = "reported.behavior")
data.regression.weights.footprint.long$behavior <- factor(data.regression.weights.footprint.long$behavior, 
                                                         levels=c( "footprint.public.transport", "footprint.diet", "footprint.electricity","footprint.flying", "footprint.recycling", "footprint.regional" ),
                                                         labels=c("commute", "diet", "electricity", "flying", "recyling", "regional") )  
  
data.regression.total1 <- merge(data.regression.weights.long_clean, data.regression.weights.policies.long, by=c("behavior", "participant.label"))

data.regression.total <- merge(data.regression.total1, data.regression.weights.footprint.long, by=c("behavior", "participant.label"))
                             
data.regression.total$impact <- factor(data.regression.total$behavior, 
                                       levels=c("commute", "diet", "electricity", "flying", "recyling", "regional"), labels=c("low", "high", "high", "high", "low","low") )

data.regression.total$domain <- factor(data.regression.total$behavior, 
                                       levels=c("commute", "diet", "electricity", "flying", "recyling", "regional"), labels=c("transport", "food", "household", "household", "transport","food") )
...
```{r}
complete.model.impact.perc <- lmer(regression.weight ~ policy.support + reported.behavior +
                            cc_concern.mean+ conservative_liberal  + impact+
                            gender + age + income + education  +
                         (1|participant.label) + (1 |behavior) ,
                          data=data.regression.total)

tab_model(complete.model.impact.perc)
plot_model(complete.model.impact.perc, show.values =TRUE, vline.color = "darkgrey")

```
#### With type (behavior) as main effect
```{r}
complete.model.impact.perc.beh <- lmer(regression.weight ~ policy.support + reported.behavior +
                                       cc_concern.mean+ conservative_liberal  + behavior +
                                       gender + age + income + education  +
                                       (1|participant.label) ,
                                   data=data.regression.total)

tab_model(complete.model.impact.perc.beh)
plot_model(complete.model.impact.perc.beh, show.values =TRUE, vline.color = "darkgrey")
...
complete.model.behavior <- lmer(reported.behavior ~  regression.weight + 
                            cc_concern.mean  +
                            gender + age + income + education  +
                         (1|participant.label) + (1 |behavior) ,
                          data=data.regression.total)

tab_model(complete.model.behavior)
plot_model(complete.model.behavior, show.values =TRUE, vline.color = "darkgrey")
plot_model(complete.model.behavior, type="emm",terms=c("regression.weight")) +
  labs(x="perceived impact", y="reported behavior")

 # also when excludcing negative regression weights it works
data.regression.total.excl.id <- data.regression.total %>% select(participant.label, regression.weight)  %>% filter(regression.weight<0) %>%  select(participant.label)
data.regression.total.excl.id <- unique(data.regression.total.excl.id)
data.regression.total.excl <- data.regression.total[!c(data.regression.total$participant.label %in% data.regression.total.excl.id$participant.label), ] 

complete.model.behavior.excl <- lmer(reported.behavior ~  regression.weight + 
                            cc_concern.mean  +
                            gender + age + income + education  +
                         (1|participant.label) + (1 |behavior) ,
                          data=data.regression.total.excl)

#tab_model(complete.model.behavior.excl)

```
#### no interaction with cc concern

```{r}
interaction.model.behavior <- lmer(reported.behavior ~  regression.weight*cc_concern.mean +
                              gender + age + income + education  +
                         (1|participant.label) + (1 |behavior) ,
                          data=data.regression.total)

tab_model(interaction.model.behavior)

interaction.model.behavior.excl <- lmer(reported.behavior ~  regression.weight*cc_concern.mean +
                              gender + age + income + education  +
                         (1|participant.label) + (1 |behavior) ,
                          data=data.regression.total.excl)

#tab_model(interaction.model.behavior.excl)

```

#### interaction of perceived impact and impactfulness of behavior
No longer significant when excluding regression weights < 0
```{r}

complete.model.behavior.impact <- lmer(reported.behavior ~  regression.weight*impact +
                            cc_concern.mean  +
                            gender + age + income + education  +
                         (1|participant.label) + (1 |behavior) ,
                          data=data.regression.total)
tab_model(complete.model.behavior.impact)
plot_model(complete.model.behavior.impact, type="emm",terms=c("regression.weight","impact"))
...
complete.model.policy<- lmer(policy.support ~ regression.weight  +
                            cc_concern.mean+ conservative_liberal +
                            gender + age + income + education  +
                         (1|participant.label) + (1|behavior) ,
                          data=data.regression.total)

tab_model(complete.model.policy)
```
#### interaction with cc concern
Not significant when excluding regression weights <0
```{r}
interaction.model.policy<- lmer(policy.support ~ regression.weight*cc_concern.mean  +
                             conservative_liberal +
                            gender + age + income + education  +
                         (1|participant.label) + (1|behavior) ,
                          data=data.regression.total.excl)

tab_model(interaction.model.policy)
plot_model(interaction.model.policy, type="pred", terms=c("regression.weight", "cc_concern.mean"))
```

#### Adding domain
```{r}
# raom intercept
complete.model.policy.domain <- lmer(policy.support ~ regression.weight + reported.behavior +
                            cc_concern.mean+ conservative_liberal  + domain +
                            gender + age + income + education  +
                         (1|participant.label) + (1 |behavior) ,
                          data=data.regression.total)

tab_model(complete.model.policy.domain)
plot_model(complete.model.policy.domain, show.values =TRUE, vline.color = "darkgrey")
...
data.regression.total.mean <- groupwiseMean(regression.weight ~ behavior ,
                                            data= data.regression.total)
data.regression.total.mean$SD <- tapply(data.regression.total$regression.weight, data.regression.total$behavior, sd)

data.regression.total.id <- groupwiseMean(regression.weight ~ behavior + participant.label,
                                            data= data.regression.total)
data.regression.total.id <- data.regression.total.id %>% rename(regression.weight = Mean)
data.regression.total.id <- merge(data.regression.total.id,data.regression.total.mean, by="behavior" )
data.regression.total.id$rel_regression.weight <- (data.regression.total.id$regression.weight -data.regression.total.id$Mean)/ data.regression.total.id$SD
data.regression.total.id$rel_regression.weight.f <- cut(data.regression.total.id$rel_regression.weight, breaks=c(-6, -1,-0.5, 0.5, 1,5))
data.regression.total.id$rel_regression.weight.f <- factor(data.regression.total.id$rel_regression.weight.f ,
                                             levels=c("(-6,-1]" , "(-1,-0.5]", "(-0.5,0.5]", "(0.5,1]",  "(1,5]" ) ,
                                             labels=c("-much lower than average", "lower than average", 
                                                      "average", "higher than average", "much higher than average"))

data.regression.total.behavior.mean <- groupwiseMean(reported.behavior ~ behavior ,
                                            data= data.regression.total)
data.regression.total.behavior.mean$SD <- tapply(data.regression.total$regression.weight, data.regression.total$behavior, sd)

data.regression.total.behavior.id <- groupwiseMean(reported.behavior ~ behavior + participant.label,
                                            data= data.regression.total)
data.regression.total.behavior.id <- data.regression.total.behavior.id %>% rename(reported.behavior = Mean)
data.regression.total.behavior.id <- merge(data.regression.total.behavior.id,data.regression.total.behavior.mean, by="behavior" )
data.regression.total.behavior.id$rel_reported.behavior <- (data.regression.total.behavior.id$reported.behavior -data.regression.total.id$Mean)/ data.regression.total.behavior.id$SD

data.regression.total.behavior.id$rel_behavior <- scale(data.regression.total.behavior.id$reported.behavior)[,1]
data.regression.total.behavior.id$rel_behavior.f <- cut(data.regression.total.behavior.id$rel_behavior, breaks=c(-1, -0.5, 0, 0.5, 1,11))
data.regression.total.behavior.id$rel_behavior.f <- factor(data.regression.total.behavior.id$rel_behavior.f ,
                                             levels=c("(-1,-0.5]" , "(-0.5,0]",  "(0,0.5]", "(0.5,1]",  "(1,11]" ) ,
                                             labels=c( "lower than average", "slightly lower than average", "slightly higher than average","higher than average", "much higher than average"))
data.regression.total.plot <- merge(data.regression.total.behavior.id, data.regression.total.id, by=c("participant.label", "behavior"))

data.regression.total.plot.flying <- data.regression.total.plot %>% filter(behavior=="flying")
plot.flying <- ggplot(data.regression.total.plot.flying, aes(x=regression.weight, fill=rel_behavior.f)) +
    geom_density(alpha=.7) +
    theme_minimal() + labs(alpha=NULL, x="impact perception", fill="carbon footprint of reported behavior", title="flying") +
  scale_fill_manual(values=c( "#127d71" , "#2F8D06","#F6BE00" ,"#DC582A", "#a50000")) + 
  scale_fill_manual(values=c( "#127d71" , "#2F8D06","#F6BE00" ,"#DC582A", "#a50000")) +
    theme(plot.caption = element_text(hjust = 0),
          plot.title.position = "plot",
...
data.regression.total.plot.diet<- data.regression.total.plot %>% filter(behavior=="diet")
plot.diet <- ggplot(data.regression.total.plot.diet, aes(x=regression.weight, fill=rel_behavior.f)) +
    geom_density(alpha=.7) +
    theme_minimal() + labs(alpha=NULL, x="impact perception", fill="carbon footprint of reported behavior", title="diet") +
...
data.regression.total.plot.commute <- data.regression.total.plot %>% filter(behavior=="commute")
plot.commute <- ggplot(data.regression.total.plot.commute, aes(x=regression.weight, fill=rel_behavior.f)) +
    geom_density(alpha=.7) +
    theme_minimal() + labs(alpha=NULL, x="impact perception", fill="carbon footprint of reported behavior", title="commute") +
       theme(legend.position = "none") +
    scale_fill_manual(values=c( "#127d71" , "#2F8D06","#F6BE00" ,"#DC582A", "#a50000"))+
...
plot.diet | plot.commute | plot.flying  
```
...
#### Commute
```{r}
model.weight.commute.fact <- lm(commute.weight ~  footprint.public.transport_fact +
                         gender + age + income + education + cc_concern.mean,
                       data=data.regression.weights)
...
data.regression.weights.t <- data.regression.weights.excl %>% rename(footprint=footprint.public.transport)
...
## with conjoint importance weights

## Using impact weights per high versus low behavior => so regression weights for low versus high also no effect, but here the variance also much smaller than for the individual behaviors

data.importance.weights.f <- data.importance.weights %>% rename(footprint=footprint.flying)
model.weight.flying.iw <- lm(flying ~ footprint +
                            gender + age + income + education + cc_concern.mean,
                          data=data.importance.weights.f)

data.importance.weights.d <- data.importance.weights %>% rename(footprint=footprint.diet)
model.weight.diet.iw <- lm(diet ~  footprint + 
                         gender + age + income + education + cc_concern.mean,
                       data=data.importance.weights.d)

data.importance.weights.e <- data.importance.weights %>% rename(footprint=footprint.electricity)
model.weight.elec.iw <- lm(electricity ~  footprint +
                         gender + age + income + education + cc_concern.mean,
                       data=data.importance.weights.e)

data.importance.weights.t <- data.importance.weights %>% rename(footprint=footprint.public.transport)
model.weight.transport.iw <- lm(commute ~ footprint +
                              gender + age + income + education + cc_concern.mean,
                            data=data.importance.weights.t)

data.importance.weights.reg <- data.importance.weights %>% rename(footprint=footprint.regional)
model.weight.regional.iw <- lm(regional ~footprint +
                       gender + age + income + education + cc_concern.mean,
                     data=data.importance.weights.reg)

data.importance.weights.rec <- data.importance.weights %>% rename(footprint=footprint.recycling)
model.weight.recycle.iw <- lm(recycling ~footprint +
                       gender + age + income + education + cc_concern.mean,
                     data=data.importance.weights.rec)
```

**Overview plot**
```{r plot.weights.iw,    message=FALSE, fig.width=25, fig.height=16}


plot_models(model.weight.flying.iw, model.weight.diet.iw, model.weight.elec.iw, model.weight.transport.iw,  model.weight.regional.iw ,dot.size = 5, line.size=1.2, show.p = TRUE, p.shape=TRUE, p.threshold = c(0.05, 0.05/6) ) +
  scale_color_viridis_d() + theme_minimal()+
   theme(plot.caption = element_text(hjust = 0),
          plot.title.position = "plot",
          plot.caption.position =  "plot",
...
data.part.worths.f <- data.part.worths %>% rename(footprint=footprint.flying)
model.weight.flying.pw <- lm(flying.weight ~ footprint +
                            gender + age + income + education + cc_concern.mean,
                          data=data.part.worths.f)

data.part.worths.d <- data.part.worths %>% rename(footprint=footprint.diet)
model.weight.diet.pw <- lm(diet.weight ~  footprint + 
                         gender + age + income + education + cc_concern.mean,
                       data=data.part.worths.d)

data.part.worths.e <- data.part.worths %>% rename(footprint=footprint.electricity)
model.weight.elec.pw <- lm(electricity.weight ~  footprint +
                         gender + age + income + education + cc_concern.mean,
                       data=data.part.worths.e)

data.part.worths.t <- data.part.worths %>% rename(footprint=footprint.public.transport)
model.weight.transport.pw <- lm(commute.weight ~ footprint +
                              gender + age + income + education + cc_concern.mean,
                            data=data.part.worths.t)

data.part.worths.reg <- data.part.worths %>% rename(footprint=footprint.regional)
model.weight.regional.pw <- lm(regional.weight ~footprint +
                       gender + age + income + education + cc_concern.mean,
                     data=data.part.worths.reg)

data.part.worths.rec <- data.part.worths %>% rename(footprint=footprint.recycling)
model.weight.recycle.pw <- lm(recycling.weight ~footprint +
                       gender + age + income + education + cc_concern.mean,
                     data=data.part.worths.rec)
```
...
**Overview plot**
```{r plot.weights.pw,    message=FALSE, fig.width=25, fig.height=16}


plot_models(model.weight.flying.pw, model.weight.diet.pw, model.weight.elec.pw, model.weight.transport.pw,  model.weight.regional.pw ,dot.size = 5, line.size=1.2, show.p = TRUE, p.shape=TRUE, p.threshold = c(0.05, 0.05/6) ) +
  scale_color_viridis_d() + theme_minimal()+
   theme(plot.caption = element_text(hjust = 0),
          plot.title.position = "plot",
          plot.caption.position =  "plot",
...
```{r}
## homoscedasticity
# weighted regression mostly makes it worse or not better
...
data.importance.weights1 <-  data.importance.weights %>% rename(weight=flying)
model.policy.flying.iw <- lm(policy.flying.tax ~  weight +
                            cc_concern.mean+ conservative_liberal +
                            gender + age + income + education ,
                           data=data.importance.weights1)

data.importance.weights2 <- data.importance.weights %>% rename(weight=commute)
model.policy.car.iw <- lm(policy.car.tax ~  weight +
                            cc_concern.mean+ conservative_liberal +
                            gender + age + income + education ,
                          data=data.importance.weights2)

data.importance.weights3 <- data.importance.weights %>% rename(weight=diet)
model.policy.meat.iw <- lm(policy.meat.tax ~ weight +
                            cc_concern.mean+ conservative_liberal +
                            gender + age + income + education ,
                      data=data.importance.weights3)

data.importance.weights4 <- data.importance.weights %>% rename(weight=electricity)
model.policy.elec.iw <- lm(policy.RE.tax ~  weight +
                            cc_concern.mean+ conservative_liberal +
                            gender + age + income + education ,
                      data=data.importance.weights4)

data.importance.weights5 <- data.importance.weights %>% rename(weight=regional)
model.policy.regional.iw <- lm(policy.regional_food.tax ~ weight +
                            cc_concern.mean+ conservative_liberal +
                            gender + age + income + education ,
                      data=data.importance.weights5)

data.importance.weights6 <- data.importance.weights %>% rename(weight=recycling)
model.policy.recycle.iw <- lm(policy.recycle.tax ~ weight +
                            cc_concern.mean+ conservative_liberal +
                            gender + age + income + education ,
                          data=data.importance.weights6)
```
...
```{r p.iw,    message=FALSE, fig.width=25, fig.height=16}
plot_models(model.policy.meat.iw, model.policy.flying.iw, model.policy.elec.iw, model.policy.car.iw,  model.policy.recycle.iw, model.policy.regional.iw ,dot.size = 5, line.size=1.2, show.p = TRUE, p.shape=TRUE, p.threshold = c(0.05, 0.05/6) )  +
  scale_color_viridis_d() +  theme_minimal()+
   theme(plot.caption = element_text(hjust = 0),
          plot.title.position = "plot",
          plot.caption.position =  "plot",
...
data.part.worths1 <-  data.part.worths %>% rename(weight=flying.weight)
model.policy.flying.pw <- lm(policy.flying.tax ~  weight +
                            cc_concern.mean+ conservative_liberal +
                            gender + age + income + education ,
                           data=data.part.worths1)

data.part.worths2 <- data.part.worths %>% rename(weight=commute.weight)
model.policy.car.pw <- lm(policy.car.tax ~  weight +
                            cc_concern.mean+ conservative_liberal +
                            gender + age + income + education ,
                          data=data.part.worths2)

data.part.worths3 <- data.part.worths %>% rename(weight=diet.weight)
model.policy.meat.pw <- lm(policy.meat.tax ~ weight +
                            cc_concern.mean+ conservative_liberal +
                            gender + age + income + education ,
                      data=data.part.worths3)

data.part.worths4 <- data.part.worths %>% rename(weight=electricity.weight)
model.policy.elec.pw <- lm(policy.RE.tax ~  weight +
                            cc_concern.mean+ conservative_liberal +
                            gender + age + income + education ,
                      data=data.part.worths4)

data.part.worths5 <- data.part.worths %>% rename(weight=regional.weight)
model.policy.regional.pw <- lm(policy.regional_food.tax ~ weight +
                            cc_concern.mean+ conservative_liberal +
                            gender + age + income + education ,
                      data=data.part.worths5)

data.part.worths6 <- data.part.worths %>% rename(weight=recycling.weight)
model.policy.recycle.pw <- lm(policy.recycle.tax ~ weight +
                            cc_concern.mean+ conservative_liberal +
                            gender + age + income + education ,
                          data=data.part.worths6)
```
...
```{r p.pw,    message=FALSE, fig.width=25, fig.height=16}
plot_models(model.policy.meat.pw, model.policy.flying.pw, model.policy.elec.pw, model.policy.car.pw,  model.policy.recycle.pw, model.policy.regional.pw ,dot.size = 5, line.size=1.2, show.p = TRUE, p.shape=TRUE, p.threshold = c(0.05, 0.05/6) )  +
  scale_color_viridis_d() +  theme_minimal()+
   theme(plot.caption = element_text(hjust = 0),
          plot.title.position = "plot",
          plot.caption.position =  "plot",
...
mode.policy.all <-  lm(cbind(policy.car.tax, policy.flying.tax,policy.meat.tax, policy.regional_food.tax, policy.RE.tax, policy.recycle.tax)  ~  
                         diet.weight +flying.weight  +commute.weight + recycling.weight + regional.weight+ electricity.weight +
                         cc_concern.mean+ conservative_liberal +
                         gender + age + income + education ,
                       data=data.regression.weights)

Anova(mode.policy.all)
...
specs_policy <- setup(data = data.regression.weights, 
               y = c("policy.car.tax", "policy.flying.tax","policy.meat.tax", "policy.regional_food.tax", "policy.RE.tax", "policy.recycle.tax"), 
               x = c( "diet.weight" , "flying.weight"  , "commute.weight" , "recycling.weight" , "regional.weight", "electricity.weight" ), 
               model = "lm", 
               controls = c("gender", "age", "education", "income") )

hyp_4 = specr(specs_policy)

plot(hyp_4, choices = c("x", "y", "controls")) 
...
specs_perception <- setup(data = data.regression.weights, 
               y = c("diet.weight" , "flying.weight"  , "commute.weight"  , "regional.weight", "electricity.weight"), 
               x = c( "footprint.diet" , "footprint.flying"  , "footprint.public.transport"  , "footprint.regional", "footprint.electricity" ), 
               model = "lm", 
               controls = c("gender", "age", "education", "income") )

hyp_3 = specr(specs_perception)

plot(hyp_3, choices = c("x", "y", "controls")) 
