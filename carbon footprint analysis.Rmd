...
```{r}
ggplot(data.accuracy.score.tau.excl, aes(x=cor.tau, fill=rel_cc_concern.mean.f, alpha=.3)) +
  geom_density(alpha=.5) +  labs(alpha=NULL, fill="z-standardized cc concern") +  
  theme_minimal() +
...
```{r}
ggplot(data.accuracy.score.tau.excl, aes(x=cor.tau, fill=income)) +
    geom_density(alpha=.7) +
    theme_minimal() + labs(alpha=NULL) +
...
```{r}
ggplot(data.accuracy.score.tau.excl, aes(x=policy.support.total, fill=rel_cor.tau.f)) +
