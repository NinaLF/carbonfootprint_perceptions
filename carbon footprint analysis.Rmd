...
ggplot(data.accuracy.score.excl, aes(x=estimate, fill=rel_cc_concern.mean.f, alpha=.3)) +
  geom_density(alpha=.5) +  labs(alpha=NULL, fill="z-standardized cc concern", x="accuracy") +  
  theme_minimal() +
...
```{r}
model.support.accuracy2 <- lm(policy.support.total ~ estimate+
                               age + gender + education + income +
                               cc_concern.mean ,
                             data=data.accuracy.score)
tab_model(model.support.accuracy2)

model.support.accuracy.excl <- lm(policy.support.total ~ estimate+
                                  age + gender + education + income +
                                  cc_concern.mean ,
                              data=data.accuracy.score.excl)
tab_model(model.support.accuracy.excl)

model.support.accuracy2.mod <- lm(policy.support.total ~ estimate*rel_cc_concern.mean+
                               age + gender + education + income ,
                             data=data.accuracy.score.excl)
...
ggplot(data.accuracy.score.excl, aes(x=policy.support.total, fill=rel_cor.f)) +
  geom_density(alpha=.5) +  labs(alpha=NULL, fill="z-standardized tau", y="policy support") +
  theme_minimal() +
  scale_fill_manual(values=c(  "#a50000", "#F6BE00", "#2F8D06"))
